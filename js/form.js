/* view-source:https://www.receita.fazenda.gov.br/Aplicacoes/SSL/ATCTA/CPF/ConsultaSituacao/ConsultaPublica.asp */
var foco = "";
var msgstatus = "";
...
function ValidarDados(xTipoSubmit) {
  ...
  if (cpf.length  != 11) {
    ...
    var data = pegaObj("txtDataNascimento").value;
    data = data.replace(/[^0-9]/g, ''); //normalizar DATA antes de fazer validação, removendo campos não numéricos
                 
    if (data.length != 8 || data.value == "00000000") {
    ...
    if (xTipoSubmit == "CaptchaSonoro") {
    if(pegaObj("txtTexto_captcha_serpro_gov_br").value.length != 6) {
      alert("Os caracteres da imagem não foram preenchidos corretamente.");
            pegaObj("txtTexto_captcha_serpro_gov_br").focus();
            return false;
    }
    } else if (xTipoSubmit == "Recaptcha") {
    if(!pegaObj("g-recaptcha-response").value) { //utilizar campo o recaptcha para fazer esta crítica
      alert("Para enviar os dados é obrigatório confirmar o reCAPTCHA.");
            pegaObj("txtDataNascimento").focus();
            return false;
        }            
    }
